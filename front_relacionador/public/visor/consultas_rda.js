...
const APIService = {
  async consultarComposition(tipoDocumento, documento) {
    ...
    const response = await fetch(`${getVisorApiBase()}/composition`, {
      method: 'POST',
  ...
  async consultarInmunizacion(tipoDocumento, documento) {
    ...
    try {
      ...
      throw new Error(rawText || `Error HTTP ${resp.status}`);
    }
    ...
    // Manejar OperationOutcome (data.error puede ser JSON FHIR o texto; no parsear si no parece JSON)
