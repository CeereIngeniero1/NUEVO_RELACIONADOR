...
AS
SELECT DISTINCT 
                         TOP (100) PERCENT fc.[Id Factura] AS IdFactura, fc.[No Factura] AS NoFactura, fc.[Id EmpresaV] AS IdEmpresaV, fc.[Fecha Factura] AS FechaFactura, CASE WHEN SUM(ctp.[Valor de Cuota Cuotas Pactadas Tratamiento]) IS NULL 
                         THEN 0 ELSE SUM(ctp.[Valor de Cuota Cuotas Pactadas Tratamiento]) END AS [Cobro a EPS], CASE WHEN SUM(cpit.[Valor de Cuota Cuotas Pactadas Inicial Tratamiento]) IS NULL 
                         THEN 0 ELSE SUM(cpit.[Valor de Cuota Cuotas Pactadas Inicial Tratamiento]) END AS Copagos, 
                         CASE WHEN fc.[Documento Responsable] <> pl.[Documento Responsable] THEN 'SS07' WHEN SUM(cpit.[Valor de Cuota Cuotas Pactadas Inicial Tratamiento]) IS NULL THEN 'SS06' ELSE 'SS01' END AS TipoFactura
FROM            dbo.[Plan de Tratamiento Tratamientos] AS pl INNER JOIN
                         dbo.Factura AS fc INNER JOIN
                         dbo.FacturaII AS fii ON fc.[Id Factura] = fii.[Id Factura] ON pl.[Id Plan de Tratamiento] = fii.[Id Plan de Tratamiento] LEFT OUTER JOIN
                         dbo.[Cuotas Pactadas Tratamiento] AS ctp ON pl.[Id Plan de Tratamiento Tratamientos] = ctp.[Id Plan de Tratamiento Tratamientos] LEFT OUTER JOIN
                         dbo.[Cuotas Pactadas Inicial Tratamiento] AS cpit ON pl.[Id Plan de Tratamiento Tratamientos] = cpit.[Id Plan de Tratamiento Tratamientos]
GROUP BY fc.[Id Factura], fc.[No Factura], fc.[Id EmpresaV], fc.[Fecha Factura], fc.[Documento Responsable], pl.[Documento Responsable]
GO


CREATE VIEW [dbo].[Face Cnsta Facturas Prepagadas]
AS
SELECT        TOP (100) PERCENT dbo.FacturaII.[Código Objeto], dbo.[Plan de Tratamiento Tratamientos].[Id Tipo Responsable], dbo.Factura.[Id Factura] AS IdFactura, dbo.Factura.[Id EmpresaV] AS IdEmpresaV, 
                         dbo.Factura.[No Factura] AS NroFactura, dbo.Factura.[Documento Responsable] AS DocResponsableFactura, dbo.[Plan de Tratamiento Tratamientos].[Documento Responsable] AS DocResponsableTratamiento, 
                         dbo.[Plan de Tratamiento].[Nro Plan de Tratamiento] AS NroPlandeTratamiento, dbo.[Cuotas Pactadas Tratamiento].[Valor de Cuota Cuotas Pactadas Tratamiento] AS ValordeCuotaCuotasPactadasTratamiento, 
                         dbo.FacturaII.[Valor FacturaII] AS ValorFacturaII, dbo.Factura.[Valor Acumulado Factura] AS ValorAcumuladoFactura, dbo.Factura.[SubTotal Factura] AS SubTotalFactura, dbo.Factura.[Total Factura] AS TotalFactura, 
                         dbo.[Plan de Tratamiento].[Documento Paciente] AS DocumentoPaciente, dbo.Entidad.[Primer Apellido Entidad] + ' ' + dbo.Entidad.[Segundo Apellido Entidad] AS ApellidoPaciente, 
                         dbo.Entidad.[Primer Nombre Entidad] + ' ' + dbo.Entidad.[Segundo Nombre Entidad] AS NombrePaciente, dbo.EntidadII.[Dirección EntidadII] AS DireccionPaciente, dbo.[Tipo de Documento].[Código Tipo de Documento], 
                         dbo.[Tipo de Documento].[Tipo de Documento] AS Tipodocumento, dbo.[Tipo de Documento].codigoDian AS CodigoTipoDocumentoPaciente, dbo.[Plan de Tratamiento].[Id Plan de Tratamiento], 
                         dbo.Departamento.[Código Departamento] AS CódigoDepartamento, dbo.Ciudad.[Código Ciudad] AS CodigoCiudad, dbo.Ciudad.Ciudad
FROM            dbo.FacturaII INNER JOIN
                         dbo.[Plan de Tratamiento] ON dbo.FacturaII.[Id Plan de Tratamiento] = dbo.[Plan de Tratamiento].[Id Plan de Tratamiento] INNER JOIN
                         dbo.[Plan de Tratamiento Tratamientos] ON dbo.[Plan de Tratamiento].[Id Plan de Tratamiento] = dbo.[Plan de Tratamiento Tratamientos].[Id Plan de Tratamiento] INNER JOIN
                         dbo.Factura ON dbo.FacturaII.[Id Factura] = dbo.Factura.[Id Factura] AND dbo.[Plan de Tratamiento].[Documento Paciente] = dbo.Factura.[Documento Paciente] INNER JOIN
                         dbo.[Cuotas Pactadas Tratamiento] ON dbo.[Plan de Tratamiento Tratamientos].[Id Plan de Tratamiento Tratamientos] = dbo.[Cuotas Pactadas Tratamiento].[Id Plan de Tratamiento Tratamientos] INNER JOIN
                         dbo.Entidad ON dbo.[Plan de Tratamiento].[Documento Paciente] = dbo.Entidad.[Documento Entidad] INNER JOIN
                         ...
                         dbo.Departamento INNER JOIN
                         dbo.Ciudad ON dbo.Departamento.[Id Departamento] = dbo.Ciudad.[Id Departamento] ON dbo.EntidadII.[Id Ciudad] = dbo.Ciudad.[Id Ciudad]
