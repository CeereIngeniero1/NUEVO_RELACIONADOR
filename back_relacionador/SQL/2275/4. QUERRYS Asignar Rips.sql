...
AS
SELECT APIRPD.IdApiRipsPorDefecto, APIRPD.DocumentoEntidad, APIRPD.TipoDeRips, TR.[Tipo Rips] AS TipoDeUsuario, Ent.[Nombre Completo Entidad] AS Entidad, RIPSVIAINU.[Nombre Via Ingreso Usuario] AS ViaIngresoServicioSalud, 
...
FROM     dbo.API_RIPS_POR_DEFECTO AS APIRPD LEFT OUTER JOIN
                  ...
                  dbo.[RIPS Via Ingreso Usuario] AS RIPSVIAINU ON APIRPD.ViaIngresoServicioSalud = RIPSVIAINU.Codigo
                       OR APIRPD.ViaIngresoServicioSalud = CAST(RIPSVIAINU.[Id Via Ingreso Usuario] AS nvarchar(20)) LEFT OUTER JOIN
                  dbo.[RIPS Modalidad Atención] AS RIPSMA ON APIRPD.ModalidadGrupoServicioTecnologiaEnSalud = RIPSMA.Codigo
                       OR APIRPD.ModalidadGrupoServicioTecnologiaEnSalud = CAST(RIPSMA.[Id Modalidad Atencion] AS nvarchar(20)) LEFT OUTER JOIN
                  dbo.[RIPS Grupo Servicios] AS RIPSGS ON APIRPD.GrupoServicios = RIPSGS.Codigo
                       OR APIRPD.GrupoServicios = CAST(RIPSGS.[Id Grupo Servicios] AS nvarchar(20)) LEFT OUTER JOIN
                  dbo.[RIPS Servicios] AS RIPSS ON APIRPD.CodigoServicio = RIPSS.[Id Servicios]
                       OR APIRPD.CodigoServicio = CAST(RIPSS.[Código Servicios] AS nvarchar(50)) LEFT OUTER JOIN
                  dbo.[RIPS Finalidad Consulta Version2] AS RIPSFCV2 ON APIRPD.FinalidadTecnologiaSalud = RIPSFCV2.Codigo
                       OR APIRPD.FinalidadTecnologiaSalud = CAST(RIPSFCV2.[Id Finalidad Consulta] AS nvarchar(20)) LEFT OUTER JOIN
                  ...
                  dbo.[RIPS Causa Externa Version2] AS RCEV2 ON APIRPD.CausaMotivoAtencion = RCEV2.Codigo
                       OR APIRPD.CausaMotivoAtencion = CAST(RCEV2.[Id RIPS Causa Externa Version2] AS nvarchar(20)) LEFT OUTER JOIN
