...
router.get('/usuarios/ripsParticular/:fechaInicio/:fechaFin/:ResolucionesRips/:documentoEmpresaSeleccionada', async (req, res) => {
    ...
    const request = new Request(
        ...
        INNER JOIN [Evaluación Entidad Rips v2] as everips ON eve.[Id Evaluación Entidad] = everips.[Id Evaluación Entidad]
    ...
    const request = new Request(
        `
          ...
          SELECT 
            ...
            EVR.Cups AS codConsulta,
            MODA.Codigo AS modalidadGrupoServicioTecSal, 
            GP.Codigo   AS grupoServicios, 
            Serv.[Código Servicios] AS codServicio,
             finv2.Codigo AS finalidadTecnologiaSalud, 
cauv2.Codigo  AS causaMotivoAtencion, 
           
            evr.Cie AS codDiagnosticoPrincipal, 
            evr.[Cie 2]  AS codDiagnosticoRelacionado1,
            evr.[Cie 3] AS codDiagnosticoRelacionado2,
            ...
            LEFT JOIN [RIPS Causa Externa Version2] as Cau on Cau.[Id RIPS Causa Externa Version2] = evr.[Id Causa Externa Version2]
            ...
            left join [Tipo de Documento] AS tpp ON Profe.[Id Tipo de Documento] = tpp.[Id Tipo de Documento]   
             LEFT JOIN [RIPS Finalidad Consulta Version2] finv2 on finv2.[Id Finalidad Consulta] = evr.[Id Finalidad Consulta Version2]
 left join [RIPS Causa Externa Version2] cauv2 on cauv2.[Id RIPS Causa Externa Version2] =  evr.[Id Causa Externa Version2]

            WHERE evr.[Id Tipo Atencion Rips] = 1 
        ...
        `
         SELECT 
			...
			ELSE  SUBSTRING(CONVERT(VARCHAR, FC.[Fecha Factura], 120), 1, 16) END AS fechaInicioAtencion, 
				...
				EVR.Cie AS codDiagnosticoPrincipal, 
				EVR.[Cie 2]   AS codDiagnosticoRelacionado, 
            ...
            fc.[SubTotal Factura] AS vrServicio,
				...
				LEFT JOIN [RIPS Finalidad Consulta Version2] as fp ON EVR.[Id Finalidad Consulta Version2] = fp.Codigo
                 LEFT JOIN [RIPS Finalidad Consulta Version2] finv2 on finv2.[Id Finalidad Consulta] = evr.[Id Finalidad Consulta Version2]
 left join [RIPS Causa Externa Version2] cauv2 on cauv2.[Id RIPS Causa Externa Version2] =  evr.[Id Causa Externa Version2]
                ...
                AND EVR.[Id Factura] = @IdFacrua
                AND EVA.[Documento Entidad] = @numDocumentoIdentificacion   
                AND CONVERT(DATE, evr.[Fecha Creacion Rips], 23) BETWEEN @fechaInicio AND @fechaFin
                ORDER BY EVR.[Id Evaluación Entidad RIPS]
