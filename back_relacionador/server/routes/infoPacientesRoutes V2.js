...
router.get('/pacientes/:fechaInicio/:fechaFin/:documentoEmpresaSeleccionada', (req, res) => {
    ...
    AND EXISTS (SELECT 1 FROM [Evaluación Entidad Rips V2] AS rips WHERE rips.[Id Evaluación Entidad] = eve.[Id Evaluación Entidad]) 
    AND  (everips.[Id Factura] IS NULL OR everips.[Id Factura] = 0)
...
router.get('/facturas/:documento', (req, res) => {
    ...
    AND NOT EXISTS (SELECT 1 FROM dbo.[Evaluación Entidad Rips V2] AS ap WHERE fc.[Id Factura] = ap.[Id Factura])
...
router.post('/relacionar', (req, res) => {
    ...
    const requestInsert = new Request('UPDATE [Evaluación Entidad Rips V2] SET [Id Factura] = @IdFactura WHERE [Id Evaluación Entidad Rips] = @IdEvaluacion', (err) => {
        if (err) {
    ...
    try {
        ...
        if (idFactura.length > 0) {
            ...
            const resultMax = await pool.request()
                .query(`SELECT MAX(ConsecutivoRipsFacturaEnCero) AS MaxConsecutivo FROM [Evaluación Entidad Rips V2]`);
            ...
            const resultInsert = await pool.request()
                ...
                .query(`UPDATE [Evaluación Entidad Rips V2] SET [Id Factura] = @IdFactura, [ConsecutivoRipsFacturaEnCero] = @NuevoConsecutivo WHERE [Id Evaluación Entidad Rips] = @IdEvaluacion`);
